...
final balanceSheetRepositoryProvider = Provider((ref) => BalanceSheetRepository());
...
final balanceSheetProvider = StateNotifierProvider<BalanceSheetNotifier, BalanceSheetState>((ref) {
...
class BalanceSheetNotifier extends StateNotifier<BalanceSheetState> {
  ...
  Future<void> loadData({DateTimeRange? range}) async {
    ...
    try {
      ...
      final transactions = await _service.getTransactions(range ?? _getDefaultRange());
      ...
      // Calculate summary
      final summary = await _service.calculateSummary(transactions, totalDue, dueCustomerCount);
